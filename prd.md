...
3. Admin klik tombol **"Verifikasi"** → **diarahkan ke halaman wizard 5 langkah** (bukan langsung membuat akun): Informasi Siswa (terisi otomatis dari data lead, **dapat dikoreksi admin**) → Paket Kelas → Mapel & Guru → Pembayaran → Preview.
...
> Perubahan produk (11 Jul 2026): pada Langkah 2 (Paket Kelas) di wizard verifikasi lead, admin sekarang memilih salah satu dari **dua menu paket**, sama seperti pada jalur wizard manual (§3.2.B): **"Paket Standar"** (pilih dari daftar `Package` yang sudah ada di data master, harga & jumlah pertemuan otomatis terisi) atau **"Paket Custom"** (admin mengisi sendiri nama paket, jenis paket, jumlah sesi, metode absensi, tipe kelas, harga dasar, status, dan deskripsi). Jika "Paket Custom" dipilih dan disimpan, sistem otomatis **membuat record `Package` baru** di data master (terhubung ke cabang & guru utama yang dipilih di Langkah 3) sebelum menautkannya ke siswa — logika ini menyamai pembuatan paket custom pada `RegistrationController@store` di jalur manual.

> Perubahan produk (13 Jul 2026): pada Langkah 2 (Paket Kelas), tampilan **"Paket Standar"** diubah dari grid kartu radio menjadi **dropdown `<select>`** tunggal. Setiap opsi dropdown menampilkan nama paket, tipe kelas, jumlah pertemuan, dan harga dalam satu baris. Saat paket dipilih, muncul info box ringkasan di bawah dropdown (nama, detail, harga). Harga total di Langkah 4 tetap otomatis terisi sesuai paket yang dipilih. Filter per cabang masih berlaku — opsi paket cabang lain tersembunyi secara otomatis saat cabang dipilih di Langkah 1. Data paket bersumber dari tabel `packages` (sama yang dikelola di `/owner/course-package`).

> Perubahan produk (11 Jul 2026, lanjutan): pada Langkah 3 (Mapel & Guru) di wizard verifikasi lead, di bawah daftar mata pelajaran kini ditampilkan ringkasan **Total Sesi** dan **Total Estimasi Biaya Guru** yang otomatis dihitung ulang (real-time) dari mapel yang dicentang, jumlah sesi, dan biaya per mapel yang diisi admin. Admin juga dapat **menambahkan mata pelajaran lain** di luar minat asli siswa melalui kontrol "Tambah Mata Pelajaran Lain" (pilih dari katalog mapel yang tersedia, lengkap dengan pilihan guru & biaya per mapel).

> Perubahan produk (11 Jul 2026, lanjutan lagi): keputusan penguncian mapel pilihan siswa di atas **dicabut** atas permintaan eksplisit — sekarang **semua mata pelajaran di Langkah 3, termasuk mapel pilihan asli siswa, dapat dihapus oleh admin** (tombol hapus tersedia di setiap baris), bukan hanya mapel tambahan. Perbaikan terkait: baris mapel yang sama tidak lagi tampil dobel ketika ada dua record `Course` dengan nama identik di data master (mis. mapel milik cabang tertentu vs mapel master global) — sistem sekarang menampilkan satu baris saja per nama mapel, memprioritaskan record yang spesifik ke cabang siswa.

> Perubahan produk (13 Jul 2026): **Langkah 3 dipecah menjadi tiga komponen/kartu terpisah** dalam satu step yang sama.
> - **Kartu A — Mata Pelajaran & Guru**: tabel dengan kolom: centang/nama mapel, guru pengajar, jumlah sesi, **Biaya Siswa (Rp)**, **Honor Guru (Rp) / Sesi**, Margin (kalkulasi otomatis), dan hapus. **Biaya Siswa (Rp) adalah nominal total untuk seluruh sesi mapel tersebut** (tidak dikali jumlah sesi), sedangkan **Honor Guru (Rp) adalah nominal per sesi** — total honor guru untuk margin/ringkasan dihitung sebagai `honor per sesi × jumlah sesi`. Setiap baris menampilkan margin (Rp + %) secara real-time saat biaya, honor, atau jumlah sesi diubah. Berisi kontrol "Tambah Mata Pelajaran Lain". Default honor guru per sesi diisi otomatis dari 60% biaya siswa dibagi jumlah sesi — dapat diedit admin. Di bawah daftar mapel terdapat 4 stat-box: **Total Sesi**, **Total Biaya Siswa**, **Total Honor Guru** (sudah dikali sesi), dan **Est. Margin Bimbel** (Rp + %) — semua diperbarui real-time.
> - **Kartu B — Jadwal Kelas**: tabel dengan 7 kolom: **No | Mata Pelajaran | Guru | Hari | Jam Mulai | Jam Berakhir | Ruang / Media**. Kolom "Guru" menampilkan nama guru yang dipilih di Kartu A secara real-time (read-only). Kolom "Ruang / Media" berupa dropdown yang datanya diambil dari tabel `rooms` (dikelola di halaman `/admin/rooms`, field `nama_ruangan` + `kapasitas`, filter status aktif & cabang). Baris bernomor urut dan otomatis terupdate saat mapel ditambah/hapus. Opsional, tidak memblokir submit.
> - **Kartu C — Cek Konflik Jadwal Guru**: panel terpisah di bawah Kartu B dengan tombol "Cek Semua Guru". Menampilkan status konflik per mapel (nama mapel + hasil cek: tersedia/bentrok/belum diisi) yang diperbarui otomatis saat guru/hari/jam diubah, atau secara massal saat tombol diklik. Menggunakan endpoint `admin.registration-list.guru-conflict-check`.
>
...
> Perubahan produk (13 Jul 2026): ditambahkan kolom **Honor Guru (Rp)** di Langkah 3 Kartu A wizard pendaftaran, terpisah dari **Biaya Siswa (Rp)**. Ini memungkinkan admin menetapkan berapa yang ditagihkan ke siswa vs berapa yang dibayarkan ke guru untuk setiap mata pelajaran. Default honor guru diisi otomatis 60% dari biaya siswa sebagai titik awal negosiasi. **Live Quotation** muncul di Langkah 5 (Preview) dengan 3 kartu: Total Tagihan Siswa, Total Honor Guru, dan Pendapatan Bimbel (margin Rp + %). Data `course_honor` dikirim ke server bersama form sebagai `course_honor[{course_id}]` untuk keperluan penggajian dan pelaporan keuangan di masa depan.
>
> Perubahan produk (13 Jul 2026, lanjutan): diklarifikasi & diperbaiki basis nominal **Honor Guru (Rp)** di Langkah 3 Kartu A — field ini merepresentasikan **honor per sesi**, bukan honor total untuk seluruh mapel (label diperjelas jadi "Honor Guru (Rp) / Sesi"). **Biaya Siswa (Rp) tetap nominal total** untuk seluruh sesi mapel tersebut (tidak dikali jumlah sesi) — jadi kedua kolom sengaja punya basis berbeda: Biaya Siswa = total, Honor Guru = per sesi. Seluruh kalkulasi turunan (Margin per baris, Total Honor Guru, Est. Margin Bimbel, serta ringkasan Live Quotation di Langkah 5) sekarang mengali honor dengan jumlah sesi mapel tersebut sebelum dijumlahkan/dikurangkan terhadap Biaya Siswa. Ini konsisten dengan kolom `honor_per_sesi` yang sudah ada di tabel `schedules` untuk penjadwalan & penggajian guru per sesi.
