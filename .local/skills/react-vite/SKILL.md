...
5. While the design subagent runs, do backend work in parallel:
    ...
    - For seed data images that don't come from a real API, use `generateImage` instead of placeholder services (DiceBear, Boring Avatars, Unsplash, Lorem Picsum, etc.). Real API image URLs (e.g. Pok--API sprites, TMDB posters) are fine. It's okay not to seed object storage.
...
6. After your backend development process is done, do not restart the frontend workflow until the design completion notification arrives, otherwise it will show a broken app. You can restart the API workflow if needed.
7. When the design completion notification arrives, fix any integration issues by calling the direct `WorkflowsRestart` tool on the existing managed workflows, then refresh logs:
