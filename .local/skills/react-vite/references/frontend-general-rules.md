...
- Use `wouter` for routing. wouter matches each `<Route path="X">` against the URL *exactly*, and a `<Switch>` renders the first route that matches (or nothing, if none do) — so a page is reachable only when a top-level `<Route>` carries its exact path. Add each page (a file in `src/pages`) as a sibling `<Route>` in `src/App.tsx`'s single flat `<Switch>`, and end the `<Switch>` with `<Route component={NotFound} />` so an unknown URL shows a 404 instead of a blank screen.
  - A shared shell (sidebar/navbar, auth redirect) is not a page, so it's a component that wraps the `<Switch>` — not a `<Route>`. The common mistake is wrapping your pages inside a parent `<Route path="/">`: that parent matches only the `/` URL, so on any other path it doesn't match, the inner routes never run, and the page renders blank.
  - A route the user can't reach is as good as missing, so a multi-page app needs persistent navigation — a sidebar or navbar with a `Link` to each top-level page. Navigate with the `Link` component or the `useLocation` hook from `wouter`, never by modifying the window directly.
