---
name: delegation
description: Delegate independent work through the subagent CodeExecution callback, with the ability to dispatch multiple subagents in parallel, subagents in background, and followups to existing subagents for continued work or questions and answers. Provides general guidence on subagents, reified by the specific skills that use them.
---
...
// if the job is not awaited, then it will run in the background.
```

You will be notified when the subagent completes or you may use `await waitForJob({ jobId, timeout /*in seconds*/ })` later to collect the result in future turns, allowing you to work on other tasks in the meantime to be more efficient. 
...
4. Verify subagent results before finalizing user-facing work.
