...
Read this when `exportSlides` returns `success: false`, or when the user reports that the slide preview pane's export button failed. The happy-path interface, `presentAsset` flow, and Google Slides redirect live in [`exporting.md`](exporting.md); this file is the troubleshooting playbook.
...
- `NO_SLIDES_FOUND` -> The `/allslides` page rendered but the exporter found no `.slide` elements. Diagnose and repair in this order, re-running the export after each fix:
  1. **Workflow not running.** Check the workflow status or logs and restart the slides workflow if it is stopped.
  2. **Empty manifest.** Read `<artifactDir>/src/data/slides-manifest.json`. If it is `[]`, the deck has no slides yet -- surface that to the user; an empty deck is unrecoverable without their intent.
  3. **Broken contract.** Read `<artifactDir>/src/App.tsx` and the slide files. Walk the `<workspace_contract>` checks in the slides skill: `/allslides` route present, each slide wrapped in `<div className="slide">` at 1920x1080, no nested `.slide` class on slide roots, wouter router, both `DO NOT edit` `useEffect`s intact, `slideLoader.ts` unchanged. Repair anything broken and run `pnpm run --filter @workspace/<slug> validate-slides`.
...
- `NO_BROWSER` -> Headless browser unavailable in the repl. Retry once; if it still fails, ask the user to retry in a moment before escalating.
...
- `INVALID_PATH` / `INVALID_REQUEST` -> Internal bug. Surface the raw `error` message -- these mean our callback constructed something the RPC rejected, which the user cannot fix.
...
When paraphrasing the raw `error` string back to the user, see the "Implementation notes -- internal-only" section in [`exporting.md`](exporting.md) for the scrub rule. Internal library names and hidden implementation details must never reach the user.
