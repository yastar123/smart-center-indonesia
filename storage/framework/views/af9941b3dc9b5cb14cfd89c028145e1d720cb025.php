...
<?php $__env->startSection('page-title','Kelola Paket Belajar'); ?>
...
function loadData(page = 1) {
    ...
    fetch(`<?php echo e(route('admin.packages.index')); ?>?${params}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
...
function editPkg(id) {
    fetch(`<?php echo e(url('admin/packages')); ?>/${id}`, { headers:{'X-Requested-With':'XMLHttpRequest'} })
        .then(r=>{ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
        .then(res => {
...
function deletePkg(id, name) {
    confirmAction(`Hapus paket "${name}"? Data tidak dapat dikembalikan.`, function() {
        fetch(`<?php echo e(url('admin/packages')); ?>/${id}`, { method:'DELETE', headers:{'X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>','X-Requested-With':'XMLHttpRequest'} })
            .then(r=>{ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
...
document.getElementById('packageForm').addEventListener('submit', function(e) {
    ...
    const url = id ? `<?php echo e(url('admin/packages')); ?>/${id}` : `<?php echo e(route('admin.packages.store')); ?>`;
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/runner/workspace/resources/views/admin/packages/index.blade.php ENDPATH**/ ?>