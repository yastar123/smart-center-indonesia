<?php $__env->startSection('title','Tryout UTBK/PTN'); ?>
<?php $__env->startSection('page-title','Tryout UTBK / PTN'); ?>
                        ...
                        <div class="dashboard-card">
                            ...
                            <form id="soalForm">
                                ...
                                <div id="pilihanSection" class="mb-3">
                                    ...
                                    <?php $__currentLoopData = ['A','B','C','D','E']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="input-group mb-1">
                                        <div class="input-group-text" style="width:40px;justify-content:center;padding:0">
                                            <input type="radio" name="kunci_jawaban" value="<?php echo e($i); ?>" title="Jawaban benar <?php echo e($opt); ?>" style="width:16px;height:16px;accent-color:#c84ddf;cursor:pointer">
                                        </div>
                                        <span class="input-group-text fw-bold" style="width:32px;justify-content:center"><?php echo e($opt); ?></span>
                                        <input type="text" name="pilihan_jawaban[]" class="form-control" placeholder="Pilihan <?php echo e($opt); ?>">
                                    </div>
...
function loadData(page = 1) {
    ...
    fetch(`<?php echo e(route('admin.tryouts.index')); ?>?${params}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
...
function editTryout(id) {
    fetch(`<?php echo e(url('admin/tryouts')); ?>/${id}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => { if (!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
        .then(res => {
...
function deleteTryout(id, name) {
    confirmAction(`Hapus tryout "${name}"? Data tidak dapat dikembalikan.`, function() {
        fetch(`<?php echo e(url('admin/tryouts')); ?>/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'X-Requested-With': 'XMLHttpRequest' } })
            .then(r => { if (!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
...
document.getElementById('tryoutForm').addEventListener('submit', function(e) {
    ...
    const url = id ? `<?php echo e(url('admin/tryouts')); ?>/${id}` : `<?php echo e(route('admin.tryouts.store')); ?>`;
...
function loadSoal(id) {
    ...
    fetch(`<?php echo e(url('admin/tryouts')); ?>/${id}/soal`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => { if (!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
        .then(res => {
...
document.getElementById('soalForm').addEventListener('submit', function(e) {
    ...
    fetch(`<?php echo e(url('admin/tryouts')); ?>/${id}/soal`, { method: 'POST', body: fd, headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
        .then(d => {
...
function deleteSoal(soalId) {
    confirmAction('Hapus soal ini? Data tidak dapat dikembalikan.', function() {
        fetch(`<?php echo e(url('admin/tryouts')); ?>/${currentTryoutId}/soal/${soalId}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'X-Requested-With': 'XMLHttpRequest' } })
            .then(r => { if (!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
...
function lihatHasil(id) {
    ...
    fetch(`<?php echo e(url('admin/tryouts')); ?>/${id}/results`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
        .then(r => { if (!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
        .then(res => {
            if (!res.data.length) {
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/runner/workspace/resources/views/admin/tryouts/index.blade.php ENDPATH**/ ?>