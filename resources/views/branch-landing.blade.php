...
<head>
    ...
    <style>
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .bl-bantuan-inner { display:grid; grid-template-columns:1fr 1fr; gap:4rem; margin-top:3.5rem; align-items:start; }
        .bl-faq-list { display:flex; flex-direction:column; gap:.7rem; }
        .bl-faq-item { border:1.5px solid rgba(200,77,223,.12); border-radius:14px; overflow:hidden; background:white; }
        .bl-faq-trigger { display:flex; align-items:center; justify-content:space-between; padding:1.1rem 1.4rem; cursor:pointer; font-size:.92rem; font-weight:600; color:var(--deep); user-select:none; transition:background .2s; background:none; border:none; width:100%; text-align:left; }
        ...
        .bl-contact-dark-card { background:linear-gradient(160deg,#260632 0%,#461256 55%,#6d1a7e 100%); border-radius:24px; padding:2.25rem 2rem; }
        .bl-contact-dark-title { font-size:1.4rem; font-weight:900; color:white; font-family:var(--font-display); margin-bottom:.4rem; }
        .bl-contact-dark-sub { font-size:.85rem; color:rgba(255,255,255,.65); margin-bottom:1.75rem; }
        .bl-contact-form-row { display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-bottom:1rem; }
        .bl-contact-field { display:flex; flex-direction:column; gap:.45rem; }
        .bl-contact-field label { font-size:.68rem; font-weight:700; color:rgba(255,255,255,.7); letter-spacing:.06em; text-transform:uppercase; }
        .bl-contact-field input,.bl-contact-field textarea { background:rgba(255,255,255,.1); border:1px solid rgba(255,255,255,.15); border-radius:10px; padding:.7rem 1rem; color:white; font-size:.88rem; outline:none; transition:border-color .2s,background .2s; width:100%; }
        .bl-contact-field input::placeholder,.bl-contact-field textarea::placeholder { color:rgba(255,255,255,.4); }
        .bl-contact-field input:focus,.bl-contact-field textarea:focus { border-color:rgba(200,77,223,.6); background:rgba(255,255,255,.14); }
        .bl-contact-field textarea { resize:none; height:110px; }
        .bl-btn-send { display:flex; align-items:center; justify-content:center; gap:8px; width:100%; padding:1rem; border-radius:12px; font-size:.95rem; font-weight:800; color:var(--deep); background:var(--gold,#f6af23); border:none; cursor:pointer; transition:transform .25s,box-shadow .25s; margin-top:1rem; }
        .bl-btn-send:hover { transform:translateY(-2px); box-shadow:0 10px 30px rgba(246,175,35,.4); }
        @media(max-width:768px) {
            .bl-bantuan-inner { grid-template-columns:1fr; gap:2.5rem; }
            .bl-contact-form-row { grid-template-columns:1fr; }
        }
                ...
                <form id="blContactForm" onsubmit="return submitBLContact(event)">
                    <div class="bl-contact-form-row">
                        <div class="bl-contact-field">
                            ...
                            <input type="text" id="blCfName" placeholder="Nama Anda" required>
                        </div>
                        <div class="bl-contact-field">
                            <label>No. WhatsApp</label>
                            <input type="tel" id="blCfPhone" placeholder="08xxxxxx" required>
                        </div>
                    </div>
                    <div class="bl-contact-field" style="margin-bottom:1rem">
                        <label>Pesan / Kebutuhan Anda</label>
                        <textarea id="blCfMsg" placeholder="Ceritakan kebutuhan belajar Anda..."></textarea>
                    </div>
                    <button type="submit" class="bl-btn-send">Kirim via WhatsApp 🚀</button>
                </form>
            </div>
        </div>
    </div>
