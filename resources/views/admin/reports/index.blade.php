...
<div class="mb-3 fade-up">
    <div class="d-flex align-items-center gap-2">
        <a href="{{ request()->fullUrlWithQuery(['tab'=>'pendapatan']) }}" class="btn btn-sm {{ $activeTab == 'pendapatan' ? 'btn-primary' : 'btn-outline-primary' }}" style="border-radius:8px;padding:6px 12px;font-size:13px">
            <i class="bi bi-cash-stack me-1"></i> Pendapatan dari Siswa
        </a>
        <a href="{{ request()->fullUrlWithQuery(['tab'=>'gaji']) }}" class="btn btn-sm {{ $activeTab == 'gaji' ? 'btn-primary' : 'btn-outline-primary' }}" style="border-radius:8px;padding:6px 12px;font-size:13px">
...
<div class="row g-4 mb-4">
    ...
    <div class="col-lg-4 fade-up" style="animation-delay:.05s">
        <div class="dashboard-card h-100">
            ...
            @php $totalInv = max($lunas + $belum + $sebagian, 1); @endphp
            ...
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    ...
                    <div class="stat-value" style="color:#dc2626;font-size:20px">
                        Rp {{ number_format($totalSalaryPaid, 0, ',', '.') }}
                    </div>
                    <div class="stat-label" style="font-size:11px">
        ...
        <div class="stat-card" style="border-top:3px solid #c84ddf">
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    <div class="stat-title">Bulan Ini</div>
                    <div class="stat-value text-primary" style="font-size:20px">
                        Rp {{ number_format($monthSalaryPaid, 0, ',', '.') }}
                    </div>
                    <div class="stat-label" style="font-size:11px">
                        <i class="bi bi-calendar-month me-1 text-primary"></i>{{ now()->locale('id')->isoFormat('MMMM Y') }}
                    </div>
        ...
        <div class="stat-card" style="border-top:3px solid #10b981">
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    ...
                    <div class="stat-value" style="color:#059669;font-size:20px">{{ $salaryPaidCount }}</div>
                    <div class="stat-label" style="font-size:11px">
        ...
        <div class="stat-card" style="border-top:3px solid #f6af23">
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    ...
                    <div class="stat-value" style="color:#e09000;font-size:28px">{{ $salaryPendingCount }}</div>
                    <div class="stat-label" style="font-size:11px">
...
<div class="row g-4 mb-4">
    ...
    <div class="col-lg-4 fade-up" style="animation-delay:.05s">
        <div class="dashboard-card h-100">
            ...
            <div class="mt-3 d-flex flex-column gap-2">
                @foreach([['Dibayar','#10b981',$salaryPaidCount],['Pending','#f6af23',$salaryPendingCount],['Batal','#ef4444',$salaryCanceledCount]] as [$lbl,$clr,$val])
                <div class="d-flex align-items-center justify-content-between" style="font-size:12.5px">
                    <div class="d-flex align-items-center gap-2">
                        <div style="width:10px;height:10px;border-radius:3px;background:{{$clr}}"></div>
                        <span>{{ $lbl }}</span>
    ...
    <div class="col-lg-7 fade-up">
        <div class="dashboard-card">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <a href="{{ route('admin.salaries.index') }}" class="btn btn-sm btn-outline-primary" style="border-radius:8px;font-size:11px;padding:4px 12px">
                    Semua <i class="bi bi-arrow-right ms-1"></i>
                </a>
            </div>
            <div class="table-responsive">
                <table class="table table-hover table-modern align-middle mb-0">
                    ...
                    <tbody>
                        @forelse($recentSalaries as $s)
                        <tr>
                            <td class="py-3">
                                <div class="fw-semibold" style="font-size:13px">{{ $s->guru?->name ?? '—' }}</div>
                                <div class="text-muted" style="font-size:11px">{{ $s->guru?->branch?->name ?? ($s->cabang?->name ?? '—') }}</div>
                            </td>
                            <td class="py-3 d-none d-md-table-cell">
                                <span class="badge" style="background:var(--input-bg);color:var(--text-muted);font-size:11px;padding:3px 8px;border-radius:6px">
                                    {{ $s->cabang?->name ?? '-' }}
                            ...
                            <td class="py-3 text-muted" style="font-size:11.5px">
                                {{ optional($s->tanggal_pembayaran)->format('d M Y') ?? optional($s->created_at)->diffForHumans() }}
                            </td>
                        </tr>
                        @empty
                        <tr>
    ...
    <div class="col-lg-5 fade-up" style="animation-delay:.05s">
        <div class="dashboard-card">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <span class="badge" style="background:var(--soft-warning-bg);color:var(--soft-warning-text);font-size:11px">{{ $salaryPendingCount }} item</span>
            </div>
            <div class="d-flex flex-column gap-2">
                @forelse($outstandingSalaries as $s)
                <div class="d-flex align-items-center justify-content-between p-3 rounded-3" style="background:var(--input-bg);border:1px solid var(--card-border)">
                    <div style="min-width:0">
                        <div class="fw-semibold text-truncate" style="font-size:12.5px;">{{ $s->guru?->name ?? '—' }}</div>
                        <div style="font-size:11px;color:var(--text-muted)">Periode: {{ $s->periode }}</div>
                    </div>
                    <div class="text-end flex-shrink-0">
                        <div class="fw-bold" style="font-size:12.5px;color:#e09000">Rp {{ number_format($s->total_gaji, 0, ',', '.') }}</div>
                        <div style="font-size:10px;color:var(--text-muted)">{{ $s->id ? 'ID#'.$s->id : '' }}</div>
                    </div>
                </div>
                @empty
                <div class="text-center py-4">
...
function renderRevenueCharts(isDark, textColor, gridColor) {
    ...
    _chartA = new ApexCharts(document.getElementById('chartRevenue'), {
        ...
        xaxis: { categories: {!! json_encode($monthlyLabels) !!}, labels:{style:{colors:textColor,fontSize:'11px'}}, axisBorder:{show:false}, axisTicks:{show:false} },
    ...
    _chartB = new ApexCharts(document.getElementById('chartInvoiceStatus'), {
        ...
        plotOptions: { pie:{ donut:{ size:'70%', labels:{ show:true, total:{ show:true, label:'Total', color:textColor, fontSize:'12px', fontWeight:600, formatter: () => '{{ $lunas + $belum + $sebagian }}' } } } } },
...
function renderSalaryCharts(isDark, textColor, gridColor) {
    ...
    _chartA = new ApexCharts(document.getElementById('chartSalary'), {
        ...
        series: [{ name:'Pengeluaran', data: {!! json_encode($salaryMonthlyData) !!} }],
        xaxis: { categories: {!! json_encode($salaryMonthlyLabels) !!}, labels:{style:{colors:textColor,fontSize:'11px'}}, axisBorder:{show:false}, axisTicks:{show:false} },
        yaxis: { labels:{ style:{colors:textColor,fontSize:'11px'}, formatter: v => 'Rp '+Intl.NumberFormat('id').format(v) } },
    ...
    _chartB = new ApexCharts(document.getElementById('chartSalaryStatus'), {
        chart: { type:'donut', height:160, fontFamily:'Inter, sans-serif', background:'transparent' },
        series: [{{ $salaryPaidCount }}, {{ $salaryPendingCount }}, {{ $salaryCanceledCount }}],
        labels: ['Dibayar','Pending','Batal'], colors: ['#10b981','#f6af23','#ef4444'], legend: { show:false },
        plotOptions: { pie:{ donut:{ size:'70%', labels:{ show:true, total:{ show:true, label:'Total', color:textColor, fontSize:'12px', fontWeight:600, formatter: () => '{{ $salaryPaidCount + $salaryPendingCount + $salaryCanceledCount }}' } } } } },
        stroke: { show:false }, dataLabels:{ enabled:false }, tooltip: { theme:isDark?'dark':'light' }
    });
    _chartB.render();
}
