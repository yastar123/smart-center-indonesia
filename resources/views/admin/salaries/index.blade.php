...
<div class="dashboard-card mb-4">
    ...
    <div class="d-flex flex-wrap gap-2" id="teacherList">
        ...
        <button type="button" class="btn btn-sm btn-outline-secondary teacher-btn" data-id="{{ $t->id }}" onclick="filterByTeacher({{ $t->id }}, this)">
            <i class="bi bi-person me-1"></i>{{ $t->name }}
            <span class="badge bg-secondary ms-1" style="font-size:10px">{{ $t->branch?->name ?? 'Pusat' }}</span>
        </button>
        @endforeach
    </div>

    {{-- SALARY TABLE below Daftar Guru --}}
    <div class="mt-4 pt-3 border-top">
        ...
        <div class="row g-2 mb-3">
            ...
            <div class="col-6 col-md-2"><select id="filterCabang" class="form-select form-select-sm"><option value="">Semua Cabang</option>@foreach($branches as $b)<option value="{{ $b->id }}">{{ $b->name }}</option>@endforeach</select></div>
    ...
    document.getElementById('tableBody').innerHTML = rows.map(s => {
        ...
        return `<tr>
            ...
            <td><div class="d-flex gap-1 justify-content-center">
                ...
                <a href="{{ url('admin/salaries') }}/${s.id}/edit" class="btn btn-sm btn-outline-secondary" title="Edit"><i class="bi bi-pencil"></i></a>
