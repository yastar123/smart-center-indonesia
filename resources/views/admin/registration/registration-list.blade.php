...
<div class="dashboard-card mb-3">
    <form method="GET" action="{{ route('admin.registration-list.index') }}" class="row g-2 align-items-end">
        <div class="col-12 col-md-4">
            <label class="form-label" style="font-size:.78rem;color:var(--text-muted)">Cari</label>
            <div class="input-group input-group-sm">
                <span class="input-group-text"><i class="bi bi-search"></i></span>
                <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Nama, HP, No. Reg...">
            </div>
        </div>
        <div class="col-6 col-md-2">
            ...
            <select name="reg_status" class="form-select form-select-sm">
                ...
                <option value="verified"  {{ request('reg_status')==='verified' ?'selected':'' }}>Terverifikasi</option>
        ...
        <div class="col-6 col-md-2">
            ...
            <select name="payment_status" class="form-select form-select-sm">
                <option value="">Semua</option>
                <option value="belum_bayar" {{ request('payment_status')==='belum_bayar'?'selected':'' }}>Belum Dibayar</option>
                <option value="cicilan"     {{ request('payment_status')==='cicilan'    ?'selected':'' }}>Cicilan</option>
        ...
        <div class="col-6 col-md-2">
            ...
            <select name="academic_status" class="form-select form-select-sm">
                <option value="">Semua</option>
                <option value="pending"        {{ request('academic_status')==='pending'        ?'selected':'' }}>Pending</option>
                <option value="menunggu_kelas" {{ request('academic_status')==='menunggu_kelas' ?'selected':'' }}>Menunggu Kelas</option>
                <option value="terjadwal"      {{ request('academic_status')==='terjadwal'      ?'selected':'' }}>Terjadwal</option>
            </select>
        </div>
        <div class="col-6 col-md-2 d-flex gap-2">
            <button type="submit" class="btn btn-primary btn-sm w-100"><i class="bi bi-funnel me-1"></i>Filter</button>
            <a href="{{ route('admin.registration-list.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x-lg"></i></a>
        </div>
    </form>
</div>
...
<div class="dashboard-card">
            ...
            <tr>
                ...
                <td style="padding:12px">
                    <div class="d-flex align-items-center gap-3">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($reg->name) }}&background={{ $reg->gender==='P'?'ec4899':'c84ddf' }}&color=fff&size=36"
                             class="rounded-circle flex-shrink-0" width="36" height="36"
                             style="border:2px solid var(--card-border)">
                        <div>
                            <div class="fw-semibold" style="font-size:.86rem">{{ $reg->name }}</div>
                            <div class="text-muted" style="font-size:.72rem;line-height:1.5">
                                ...
                                @if($reg->program)<span class="badge" style="background:var(--soft-primary-bg);color:var(--soft-primary-text);padding:2px 7px;border-radius:10px;font-size:.65rem">{{ $reg->program }}</span>@endif
                            </div>
                            @if($reg->interests && count($reg->interests))
                            <div class="mt-1 d-flex flex-wrap gap-1">
                                @foreach(array_slice($reg->interests, 0, 3) as $int)
                                <span style="font-size:.65rem;background:var(--input-bg);border:1px solid var(--card-border);padding:1px 6px;border-radius:8px;color:var(--text-muted)">{{ $int }}</span>
                                @endforeach
                                @if(count($reg->interests) > 3)<span style="font-size:.65rem;color:var(--text-muted)">+{{ count($reg->interests)-3 }} lainnya</span>@endif
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                <td style="padding:12px">
                    <div>
                        <code style="font-size:.72rem;background:var(--input-bg);padding:2px 6px;border-radius:6px">{{ $reg->no_reg }}</code>
                    </div>
                    ...
                    <div class="mt-1">
                        <span class="badge" style="background:{{ $regBadge[0] }};color:{{ $regBadge[1] }};padding:2px 8px;border-radius:10px;font-size:.65rem">
                            {{ $regBadge[2] }}
                        </span>
                    </div>
                </td>
                <td style="padding:12px">
                    <span class="badge d-inline-flex align-items-center gap-1"
                          style="background:{{ $payBadge[0] }};color:{{ $payBadge[1] }};padding:5px 10px;border-radius:20px;font-size:.75rem;font-weight:500">
                        <i class="bi {{ $payBadge[3] }}" style="font-size:.72rem"></i>
                        {{ $payBadge[2] }}
                    </span>
                    @if($reg->total_biaya)
                    <div class="text-muted mt-1" style="font-size:.72rem">Rp{{ number_format($reg->total_biaya,0,',','.') }}</div>
                    @endif
                </td>
                <td style="padding:12px">
                    <span class="badge d-inline-flex align-items-center gap-1"
                          style="background:{{ $acadBadge[0] }};color:{{ $acadBadge[1] }};padding:5px 10px;border-radius:20px;font-size:.75rem;font-weight:500">
                        <i class="bi {{ $acadBadge[2] }}" style="font-size:.72rem"></i>
                        {{ $acadBadge[1+1] }}
                    </span>
                </td>
                <td style="padding:12px;text-align:right">
                    <div class="d-flex justify-content-end gap-2 flex-wrap">
                        @if($reg->status === 'verified' && $reg->status !== 'rejected')
                        <a href="{{ route('admin.registration-list.approve', $reg->id) }}"
    ...
    <div class="d-flex justify-content-between align-items-center mt-3 pt-3" style="border-top:1px solid var(--card-border)">
        <div class="text-muted" style="font-size:.8rem">
            Menampilkan {{ $registrations->firstItem() }}–{{ $registrations->lastItem() }} dari {{ $registrations->total() }}
        </div>
        {{ $registrations->links() }}
    </div>
    @endif
