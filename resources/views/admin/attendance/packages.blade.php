@extends('layouts.app')
@section('title', 'Absensi — Daftar Paket')
...
<div class="dashboard-card mb-4 fade-up" style="background:linear-gradient(135deg,#260632 0%,#461256 50%,#c84ddf 100%);color:white;border:none;overflow:hidden;position:relative">
    ...
    <div class="row align-items-center g-3" style="position:relative">
        ...
        <div class="col-md-4 text-md-end">
            <a href="{{ route('admin.attendance.sessions') }}" class="btn fw-semibold px-4"
...
<div class="row g-3 mb-4">
    <div class="col-6 col-lg-3 fade-up">
        <div class="stat-card" style="border-top:3px solid #c84ddf">
            <div class="d-flex justify-content-between align-items-start">
                <div><div class="stat-title">Total Paket</div><div class="stat-value">{{ $pakets->count() }}</div></div>
    ...
    <div class="col-6 col-lg-3 fade-up" style="animation-delay:.05s">
        <div class="stat-card" style="border-top:3px solid #10b981">
            <div class="d-flex justify-content-between align-items-start">
                <div><div class="stat-title">Total Sesi</div><div class="stat-value text-success">{{ $pakets->sum('total_sesi') }}</div></div>
    ...
    <div class="col-6 col-lg-3 fade-up" style="animation-delay:.10s">
        <div class="stat-card" style="border-top:3px solid #0284c7">
            <div class="d-flex justify-content-between align-items-start">
                <div><div class="stat-title">Rekap Hadir</div><div class="stat-value text-primary">{{ $pakets->sum('hadir_count') }}</div></div>
    ...
    <div class="col-6 col-lg-3 fade-up" style="animation-delay:.15s">
        <div class="stat-card" style="border-top:3px solid #ef4444">
            <div class="d-flex justify-content-between align-items-start">
                <div><div class="stat-title">Rekap Alpa</div><div class="stat-value text-danger">{{ $pakets->sum('alpa_count') }}</div></div>
...
<div class="dashboard-card fade-up">
    <div class="d-flex align-items-center justify-content-between mb-3">
        <h6 class="fw-bold mb-0">
            ...
            <span class="badge ms-2" style="background:var(--soft-primary-bg);color:var(--soft-primary-text);font-size:11px">{{ $pakets->count() }} paket</span>
        </h6>
    </div>
        ...
        <table class="table table-hover table-modern align-middle mb-0">
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid var(--card-border);cursor:pointer"
                    onclick="window.location='{{ route('admin.attendance.sessions') }}?paket_id={{ $p->id }}'">
                    <td class="ps-3">
                        <div class="fw-semibold" style="font-size:13px">{{ $p->nama }}</div>
                        @if($mapel)
                        <div class="text-muted" style="font-size:11px"><i class="bi bi-journal-bookmark me-1"></i>{{ \Illuminate\Support\Str::limit($mapel, 50) }}</div>
                        @endif
                        @if($p->total_sesi > 0)
                        <div class="mt-1">
                            <div style="height:4px;background:var(--card-border);border-radius:2px;overflow:hidden;width:120px">
                                <div style="height:100%;width:{{ $progress }}%;background:linear-gradient(90deg,#c84ddf,#461256);border-radius:2px;transition:width .6s ease"></div>
                            </div>
                            <span style="font-size:10px;color:var(--text-muted)">{{ $progress }}% selesai</span>
                        </div>
                        @endif
                    </td>
                    <td>
                        <span class="badge" style="background:{{ $jm['bg'] }};color:{{ $jm['color'] }};font-size:11px;padding:4px 10px;border-radius:8px">
                            {{ $jm['label'] }}
                        </span>
                    </td>
                    <td style="font-size:12.5px">{{ $p->cabang?->name ?? '—' }}</td>
                    <td style="font-size:12.5px">{{ $p->guru?->name ?? '—' }}</td>
                    <td class="text-center">
                        <span class="fw-bold" style="font-size:13px">{{ $p->total_sesi }}</span>
                    </td>
                    <td class="text-center">
                        <span class="fw-semibold text-success" style="font-size:13px">{{ $p->selesai_sesi }}</span>
                    </td>
                    <td class="text-center">
                        @if($p->hadir_count > 0)
                        <span style="color:var(--soft-success-text);font-size:12px;font-weight:600">
                            <i class="bi bi-check-circle-fill me-1"></i>{{ $p->hadir_count }}
                        </span>
                        @else
                    ...
                    <td class="text-center">
                        ...
                        <span style="color:var(--soft-danger-text);font-size:12px;font-weight:600">
                            <i class="bi bi-x-circle-fill me-1"></i>{{ $p->alpa_count }}
                        </span>
                        @else
                        <span class="text-muted" style="font-size:12px">0</span>
                        @endif
                    ...
                    <td class="text-center" onclick="event.stopPropagation()">
                        <a href="{{ route('admin.attendance.sessions') }}?paket_id={{ $p->id }}"
