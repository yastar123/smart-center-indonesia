    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3" style="position:relative">
        ...
        <div class="d-flex gap-3">
            <div style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.2);border-radius:14px;padding:14px 20px;text-align:center;backdrop-filter:blur(4px);min-width:90px">
                <div style="font-size:28px;font-weight:800;color:white;line-height:1">{{ $todayClassCount }}</div>
                <div style="font-size:11px;opacity:.75;margin-top:4px">Kelas Hari Ini</div>
            </div>
            <div style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.2);border-radius:14px;padding:14px 20px;text-align:center;backdrop-filter:blur(4px);min-width:90px">
                <div style="font-size:28px;font-weight:800;color:{{ $pendingJurnals > 0 ? '#fbbf24' : 'white' }};line-height:1">{{ $pendingJurnals }}</div>
...
<div class="dashboard-card fade-up" style="animation-delay:.06s">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <span class="text-muted" style="font-size:12px">{{ $todayClassCount }} Sesi · {{ now()->locale('id')->isoFormat('dddd, D MMMM Y') }}</span>
        </div>
    ...
    <div class="d-flex flex-column gap-3">
        ...
        <div class="d-flex gap-0" style="position:relative">
            ...
            <div class="d-flex flex-column align-items-center me-3" style="flex-shrink:0;width:40px">
                <div style="width:36px;height:36px;border-radius:50%;background:{{ $isNow ? 'linear-gradient(135deg,#10b981,#059669)' : ($isPast ? 'var(--soft-muted-bg)' : 'linear-gradient(135deg,#461256,#c84ddf)') }};display:flex;align-items:center;justify-content:center;color:white;font-size:14px;font-weight:700;flex-shrink:0;z-index:1">
                    {{ $idx + 1 }}
                </div>
                @if(!$loop->last)
                ...
                <div class="p-3">
                    <div class="d-flex align-items-start justify-content-between gap-2 mb-2">
                        ...
                        <div class="d-flex align-items-center gap-2 flex-shrink-0">
                            ...
                            @endif
                            <span class="badge d-flex align-items-center gap-1" style="background:{{ $statusBg }};color:{{ $statusColor }};font-size:11px;padding:4px 10px;border-radius:20px;{{ $isNow ? 'animation:pulseDot 2s ease-in-out infinite' : '' }}">
                                <i class="bi {{ $statusIcon }}" style="font-size:{{ $isNow ? '7px' : '10px' }}"></i>
                                {{ $isNow ? 'LIVE' : '' }} {{ $statusLabel }}
                            </span>
                        </div>
                    </div>
                    ...
                    @if($isNow || ($isPast && $s->status !== 'selesai'))
