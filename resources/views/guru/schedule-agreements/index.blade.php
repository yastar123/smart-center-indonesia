...
<div class="dashboard-card mb-4 fade-up"
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3" style="position:relative">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <div style="font-size:11px;opacity:.6;margin-bottom:3px;text-transform:uppercase;letter-spacing:.08em">Portal Guru</div>
                <h4 class="fw-bold mb-1" style="color:white;letter-spacing:-.02em">Persetujuan Jadwal</h4>
                <p class="mb-0" style="opacity:.75;font-size:13px">Ajukan jadwal pertemuan baru · Semua pihak harus menyetujui sebelum jadwal dibuat</p>
            </div>
        </div>
        <div>
...
<div class="dashboard-card mb-4 fade-up" style="border-left:4px solid #f6af23;background:rgba(246,175,35,.05)">
    <div class="d-flex align-items-center gap-2">
        ...
        <div>
            ...
            <div style="font-size:12px;color:var(--text-muted)">Guru hanya dapat mengajukan perubahan jadwal maksimal <strong>1 hari sebelum</strong> jadwal yang akan diubah berlangsung.</div>
        </div>
    </div>
</div>
...
<div class="row g-3 mb-4">
    <div class="col-6 col-lg-3 fade-up">
        <div class="stat-card" style="border-top:3px solid #c84ddf">
            <div class="d-flex justify-content-between align-items-start">
                <div><div class="stat-title">Total Proposal</div><div class="stat-value">{{ $stats['total'] }}</div></div>
    ...
    <div class="col-6 col-lg-3 fade-up" style="animation-delay:.05s">
        <div class="stat-card" style="border-top:3px solid #f6af23">
            <div class="d-flex justify-content-between align-items-start">
                <div><div class="stat-title">Menunggu Respons</div><div class="stat-value text-warning">{{ $stats['pending'] }}</div></div>
    ...
    <div class="col-6 col-lg-3 fade-up" style="animation-delay:.10s">
        <div class="stat-card" style="border-top:3px solid #10b981">
            <div class="d-flex justify-content-between align-items-start">
                <div><div class="stat-title">Disetujui</div><div class="stat-value text-success">{{ $stats['approved'] }}</div></div>
    ...
    <div class="col-6 col-lg-3 fade-up" style="animation-delay:.15s">
        <div class="stat-card" style="border-top:3px solid #ef4444">
            <div class="d-flex justify-content-between align-items-start">
                <div><div class="stat-title">Ditolak</div><div class="stat-value text-danger">{{ $stats['rejected'] }}</div></div>
...
<div class="dashboard-card fade-up">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="fw-bold mb-0"><i class="bi bi-list-ul text-primary me-2"></i>Daftar Proposal Jadwal
            <span class="badge ms-2" style="background:var(--soft-primary-bg);color:var(--soft-primary-text);font-size:11px">{{ $proposals->total() }} data</span>
        </h6>
    </div>
        ...
        <table class="table table-hover table-modern align-middle mb-0">
            ...
            <tbody>
                ...
                @php
                    ...
                    $myApproval = $proposal->approvals->where('approver_type', 'guru')->where('approver_id', $teacher->id)->first();
                @endphp
                <tr style="border-bottom:1px solid var(--card-border)">
                    ...
                    <td>
                        ...
                        <div class="text-muted" style="font-size:11px">{{ $proposal->proposed_by_type === 'guru' ? 'Guru' : 'Siswa' }}</div>
                    </td>
                    <td>
                        <div class="fw-semibold" style="font-size:13px">{{ $proposal->tanggal ? $proposal->tanggal->format('d M Y') : '–' }}</div>
                        <div class="text-muted" style="font-size:11px"><i class="bi bi-clock me-1"></i>{{ substr($proposal->jam_mulai,0,5) ?? '–' }} – {{ substr($proposal->jam_selesai,0,5) ?? '–' }}</div>
                    </td>
                    <td>
                    ...
                    <td>
                        <div style="font-size:12px">
                            <div class="mb-1"><i class="bi bi-people-fill me-1"></i>{{ $proposal->approvedCount() }}/{{ $proposal->approvals->count() }} setuju</div>
                            @if($proposal->pendingCount() > 0)
                            <div class="text-muted" style="font-size:11px">{{ $proposal->pendingCount() }} menunggu</div>
                            @endif
                        </div>
                    </td>
                    <td class="text-center">
                        @if($proposal->status === 'pending' && $myApproval && $myApproval->status === 'pending')
                        <div class="d-flex justify-content-center gap-1">
                            <button onclick="respondProposal({{ $proposal->id }}, 'approved')" class="btn btn-sm btn-success" style="border-radius:8px;padding:5px 10px">
                                <i class="bi bi-check-lg"></i>
                            </button>
                            <button onclick="respondProposal({{ $proposal->id }}, 'rejected')" class="btn btn-sm btn-danger" style="border-radius:8px;padding:5px 10px">
    ...
    <div class="mt-4 d-flex justify-content-center">{{ $proposals->links() }}</div>
    @endif
    @endif
</div>
                        ...
                        <div class="col-12">
                            ...
                            <select id="class_id" name="class_id" class="form-select" style="border-radius:10px" required onchange="onClassChange(this.value)">
                                ...
                                <option value="{{ $c->id }}"
                                    ...
                                    {{ $c->nama_kelas }} — {{ $c->mataPelajaran?->nama ?? '' }}
                                </option>
                                @endforeach
                            </select>
                        </div>
...
function onClassChange(classId) {
    ...
    fetch(`/guru/schedule-agreements/class/${classId}/meetings`, {
...
function saveProposal() {
    ...
    $.ajax({
        url: '{{ route('guru.schedule-agreements.store') }}',
        method: 'POST',
...
function respondProposal(proposalId, status) {
    ...
    confirmAction(`Anda yakin ingin ${actionText} proposal jadwal ini?`, function() {
        $.ajax({
            url: status === 'approved' ? `/guru/schedule-agreements/${proposalId}/approve` : `/guru/schedule-agreements/${proposalId}/reject`,
            method: 'POST',
