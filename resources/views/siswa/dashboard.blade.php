...
<div class="dashboard-card mb-4 fade-up" style="padding-bottom:20px">
    <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <a href="{{ route('siswa.courses.fees') }}" class="text-primary fw-semibold" style="font-size:12px;text-decoration:none">
    ...
    <div class="d-flex gap-3 overflow-auto pb-2" style="scrollbar-width:thin">
        ...
        <div class="flex-shrink-0 p-3 rounded-3 position-relative overflow-hidden"
            ...
            <a href="{{ route('siswa.courses.fees') }}" class="btn btn-sm fw-semibold"
        ...
        <div class="flex-shrink-0 p-3 rounded-3 position-relative overflow-hidden"
            ...
            <a href="{{ route('siswa.schedule-agreements.index') }}" class="btn btn-sm btn-outline-primary fw-semibold"
        ...
        <div class="flex-shrink-0 p-3 rounded-3 position-relative overflow-hidden"
            ...
            <div class="fw-bold mb-3" style="font-size:14px;color:#c84ddf;letter-spacing:.05em;font-family:monospace">{{ $referralCode }}</div>
            <button onclick="navigator.clipboard.writeText('{{ $referralCode }}').then(()=>showToast('Kode disalin!','success'))"
...
{{-- MINI STATS + KELAS TERDEKAT (row) --}}
<div class="row g-4 mb-4">

    {{-- LEFT: Kelas Terdekat + Mini Stats --}}
                ...
                <div class="stat-card" style="border-top:3px solid #c84ddf">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <div class="stat-value text-primary" data-auto-count="{{ $sisaKuota }}">{{ $sisaKuota }}</div>
                ...
                <div class="stat-card" style="border-top:3px solid #10b981">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            ...
                            <div class="stat-value text-success" data-auto-count="{{ $sesiSelesai }}">{{ $sesiSelesai }}</div>
                            <div class="text-muted" style="font-size:11px">dari {{ $totalSesi ?: '?' }} total</div>
                        </div>
        ...
        <div class="dashboard-card h-auto">
            ...
            <div class="p-3 rounded-3 mb-3" style="background:linear-gradient(135deg,rgba(200,77,223,.07),rgba(70,18,86,.05));border:1.5px solid rgba(200,77,223,.18)">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <div>
                        <div class="fw-bold" style="font-size:14px;color:var(--text-primary)">{{ $kelasNm }}</div>
                        <div class="text-muted" style="font-size:12px">{{ $guruNm }}</div>
                    </div>
                    @if($isToday)
                    ...
                    @else
                    <span class="badge" style="background:var(--soft-muted-bg);color:var(--soft-muted-text);font-size:10px;border-radius:6px">{{ $tglStr }}</span>
                    @endif
                </div>
                <div class="d-flex align-items-center gap-3" style="font-size:12px;color:var(--text-muted)">
                    <span><i class="bi bi-clock me-1"></i>{{ substr($nc->jam_mulai,0,5) }} – {{ substr($nc->jam_selesai,0,5) }}</span>
                    <span>·</span>
                    <span><i class="bi bi-geo-alt me-1"></i>{{ $room }}</span>
                </div>
            </div>

            @if($nextPending)
            <div class="d-flex gap-2 mb-3">
                <form method="POST" action="{{ route('siswa.attendance.confirm', $nextClass->id) }}" class="flex-fill">
                    @csrf
            ...
            @else
            <div class="d-flex gap-2 mb-3">
                <a href="{{ route('siswa.attendance') }}" class="btn btn-success fw-bold flex-fill" style="border-radius:10px;font-size:13px;opacity:.45" disabled>
                    <i class="bi bi-check-lg me-1"></i>HADIR
                </a>
                <a href="{{ route('siswa.attendance') }}" class="btn btn-outline-secondary fw-bold flex-fill" style="border-radius:10px;font-size:13px;opacity:.45">
                    <i class="bi bi-x-lg me-1"></i>TDK HADIR
                </a>
            </div>
    ...
    {{-- RIGHT: Paket Aktif + Riwayat Terakhir --}}
        ...
        <div class="dashboard-card mb-3">
            ...
            <div class="d-flex align-items-start gap-3 mb-3">
                <div style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#461256,#c84ddf);color:white;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:700;flex-shrink:0">
                    {{ strtoupper(mb_substr(preg_replace('/[^A-Za-z]/','', $activeKelas->mataPelajaran?->nama ?? $activeKelas->nama_kelas),0,2)) ?: 'KL' }}
                </div>
                <div style="flex:1;min-width:0">
                    <div class="fw-bold text-truncate" style="font-size:14px;color:var(--text-primary)">{{ $activeKelas->nama_kelas }}</div>
                    <div class="text-muted" style="font-size:12px">
                        {{ $subjectNames->take(3)->join(' & ') }}
                        @if($subjectNames->count() > 3) <span style="color:var(--soft-primary-text)">+{{ $subjectNames->count()-3 }} lainnya</span> @endif
                    </div>
                </div>
            ...
            <div class="mb-1 d-flex justify-content-between align-items-center" style="font-size:12px">
                ...
                <span class="fw-bold" style="color:#c84ddf">{{ $progresKelas }}%</span>
            </div>
            <div style="height:8px;background:var(--card-border);border-radius:99px;overflow:hidden;margin-bottom:6px">
                <div style="height:100%;width:{{ $progresKelas }}%;background:linear-gradient(90deg,#461256,#c84ddf);border-radius:99px;transition:width .5s"></div>
            </div>
            <div class="d-flex justify-content-between align-items-center">
                <div class="text-muted" style="font-size:12px">{{ $sesiSelesai }} dari {{ $totalSesi ?: '?' }} Pertemuan Selesai</div>
                <a href="{{ route('siswa.kelas.index') }}" class="btn btn-sm btn-primary fw-semibold" style="border-radius:8px;font-size:11px">
        ...
        <div class="dashboard-card">
            ...
            <div class="d-flex flex-column gap-2">
                ...
                <div class="d-flex align-items-center gap-3 p-3 rounded-3" style="background:var(--input-bg);border:1px solid var(--card-border)">
                    ...
                    <div style="flex:1;min-width:0">
                        <div class="fw-semibold" style="font-size:13px;color:var(--text-primary)">Hadir: {{ $lastAttendance->mapel ?? 'Kelas' }}</div>
                        <div class="text-muted" style="font-size:11px">
                            {{ $lastAttendance->tanggal ? \Carbon\Carbon::parse($lastAttendance->tanggal)->locale('id')->isoFormat('dddd, D MMM Y') : '—' }}
                        </div>
                    </div>
                </div>
                @else
                ...
                <div class="d-flex align-items-center gap-3 p-3 rounded-3" style="background:var(--input-bg);border:1px solid var(--card-border)">
                    ...
                    <div style="flex:1;min-width:0">
                        ...
                        <div class="text-muted" style="font-size:11px">
                            {{ $lastInvoice->nomor_invoice }} · Rp {{ number_format($lastInvoice->total, 0, ',', '.') }}
                        </div>
                    </div>
                    <a href="{{ route('siswa.billing.index') }}" class="btn btn-sm btn-outline-primary flex-shrink-0" style="font-size:11px;border-radius:8px">Lihat</a>
                </div>
                @endif
            </div>
        </div>
