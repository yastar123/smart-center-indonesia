...
<div class="page">
    ...
    <div class="inner">
        ...
        <div class="cert-title">{{ strtoupper($certificate->jenis) }}</div>
        <div class="cert-body">Diberikan kepada:</div>
        <div class="student-name">{{ $student->name ?? $certificate->siswa->user->name ?? 'Peserta' }}</div>
        <div class="cert-body">Telah berhasil menyelesaikan program <strong>{{ $certificate->judul }}</strong><br>
        @if($certificate->deskripsi){{ $certificate->deskripsi }}@endif</div>
        @if($certificate->tanggal_terbit)
        <div style="font-size:12px;color:rgba(255,255,255,.65)">Diterbitkan pada {{ $certificate->tanggal_terbit->format('d F Y') }}</div>
        @endif
        <div class="cert-number">No. {{ $certificate->nomor_sertifikat }}</div>
